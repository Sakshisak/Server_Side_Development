...
}, (accessToken, refreshToken, profile, done) => {      //callback fn with these 3 values
    User.findOne({facebookId: profile.id}, (err, user) => {     //profile contains info given by fb
        if (err) {
            return done(err, false);
        }
        if (!err && user !== null) {
            return done(null, user);    //if the user has already logged in, we just return the user
        }
